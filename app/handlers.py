...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_photo(
        ...
        caption=(
            "⚠️ **تنويه هام** ⚠️\n"
            "**قد يتأخر رد البوت أحيانًا لمدّة لا تتجاوز دقيقة واحدة نتيجة الضغط على النظام.**\n"
            "يرجى الانتظار قليلًا أو إعادة المحاولة بعد أقل من دقيقة، وسيتم الرد تلقائيًا فور معالجة الطلب.\n\n"
              "⚠️  اي تاخير اكثر من ذلك انتقل لتفعيل التطبيق من خلال ارسال رمز النسخة الكاملة لتطبيقك عبر مراسلتنا على الواتساب من داخل  التطبيق :\n\n"
            "🔹 طرق الدفع المتاحة:\n\n"

            "1️⃣ عبر *الشام كاش*:\n"
            "   ▪️ الدفع يتم بإحدى الطريقتين:\n"
            "      - إما بمسح صورة باركود حساب الشام كاش الخاص بنا الموجودة في الاعلى وهي الطريقة الاسهل.\n"
            "      - أو إدخال عنوان باركود الشام كاش الخاص بنا التالي:\n"
            "`ce95cda303cc0c382736307089e2ddeb`\n\n"

            "2️⃣ عبر *سيريتل كاش*:\n"
            "   ▪️ التحويل يتم *كاش عبر التحويل اليدوي* إلى الرقم التالي:\n"
            "`0997625546`\n"
            "   ⚠️ ملاحظة هامة: هذا التحويل هو *كاش يدوي مباشر* وليس تعبئة رصيد وحدات للخط.\n\n"

            "📌الخطوة التالية :\n"
            "▪️ انسخ رمز النسخة الكاملة الخاص بجهازك من التطبيق بالضغط على نسخ الرمز.\n\n"

            "⬇️ الصقه هنا في الدردشة التالية وارسله لنا للانتقال إلى الخطوة التالية ⬇️"
        )
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "device_id" not in context.user_data:
        ...
        await update.message.reply_text("📸 أرسل الآن صورة إشعار الدفع من خلال عمل لقطة شاشة أو نص إشعار الدفع أو رقم عملية الدفع.")
        return
    ...
    if "notify_msg" not in context.user_data:
        ...
        await update.message.reply_text(
            "✅ ممتاز   .\n"
            "اضغط ارسال  لاستلام بيانات الدفع الخاصة بك.",
...
async def team_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "new_order":
        context.user_data.clear()
        await query.message.reply_text("📱 أدخل رمز النسخة الكاملة الخاصة بجهازك المنسوخة مسبقا من خلال الضغط على زر نسخ الرمز  من داخل التطبيق.")
        return
